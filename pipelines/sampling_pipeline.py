...
def sample_generator(max_blokk_volume=None, cube_size=3):
    ...
    for partition_samples in combinatorics.generate_all_blokk_samples_by_partition():
        sample = [
            {
                "idx": partition_samples["idx"],
                "sample": sorted(s),
            }
            for s in partition_samples["samples"]
        ]
        yield sample


def run_pipeline(
    database_name=None, dataset_name=None, max_blokk_volume=None, cube_size=3
):
    # some defaults
    if dataset_name is None:
        if max_blokk_volume is None or max_blokk_volume >= 5:
            ...
            dataset_name = f"cube_{cube_size}_v{max_blokk_volume}"
